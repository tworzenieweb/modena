	...
	*/
	function widget($args, $instance){
		...
		if (strlen($pageURL) > 23) {
			...
			$like_box_iframe = <<<HTML
<iframe src="http://www.facebook.com/plugins/likebox.php?href={$pageURL}&amp;width=80%&amp;colorscheme={$colorScheme}&amp;show_border=false&amp;show_faces={$showFaces}&amp;connections={$connection}&amp;stream={$streams}&amp;header={$header}&amp;height={$height}"
    "scrolling="no"
    frameborder="0"
    style="border:none; overflow:hidden; width:250px; height:{$height}px;" allowTransparency="true">
    </iframe>
HTML;
			$like_box_xfbml = <<<HTML
<script src="http://connect.facebook.net/pl_PL/all.js#xfbml=1"></script>
    <fb:like-box href="{$pageURL}" width="250" show_faces="{$showFaces}" show_border="false" stream="{$streams}" header="{$header}">
    </fb:like-box>
HTML;
		} else {
			$like_box_iframe = "<iframe src=\"http://www.facebook.com/plugins/likebox.php?id=$pageID&amp;width=100%&amp;colorscheme=$colorScheme&amp;border_color=$borderColor&amp;show_faces=$showFaces&amp;connections=$connection&amp;stream=$streams&amp;header=$header&amp;height=$height\" scrolling=\"no\" frameborder=\"0\" style=\"border:none; overflow:hidden; width:" . $width . "px; height:" . $height . "px;\" allowTransparency=\"true\"></iframe>";
			$like_box_xfbml = "<script src=\"http://connect.facebook.net/pl_PL/all.js#xfbml=1\"></script><fb:like-box id=\"$pageID\" width=\"100%\" show_faces=\"$showFaces\" show_border=\"false\" stream=\"$streams\" header=\"$header\"></fb:like-box>";		
		}
		$like_button_xfbml  = "<script src=\"http://connect.facebook.net/pl_PL/all.js#xfbml=1\"></script><fb:like layout=\"$fblike_button_style\" show_faces=\"$fblike_button_showFaces\" width=\"$fblike_button_width\" action=\"$fblike_button_verb_to_display\" font=\"$fblike_button_font\" colorscheme=\"$fblike_button_colorScheme\"></fb:like>";
		$html = ""; 
